...
def ingest_invoice_pdf(file_stream, api_key: str) -> dict:
    """
    Ingests an invoice PDF from a file stream, extracts text, and uses a user-provided
    API key with an LLM to extract key information in a structured format.
    """
    ...
    for date_format in possible_formats:
        try:
            validated_date = datetime.strptime(extracted_data['invoice_date'], date_format).date()
